...
var (
	...
	enableNFSv3    = flag.Bool("enable-nfs-v3", true, "If the NFS server should serve the NFSv3 protocol and its ancillary services (rpcbind, statd, mountd, nlockmgr, rquotad). When false the server is NFSv4-only: rpcbind/statd are not started, ganesha is configured with NFS_Protocols=4, Enable_UDP=false, and Enable_RQUOTA=false so it serves only NFSv4 over 2049/TCP with no portmapper, and the provisioner validates its own Service against just that single port. Default true.")
)
