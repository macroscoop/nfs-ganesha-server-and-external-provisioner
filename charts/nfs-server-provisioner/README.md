...
| `nfsV3.enabled`                | Expose the NFSv3 protocol and its ancillary services (rpcbind, statd, mountd, nlockmgr, rquotad). Set to `false` for an NFSv4-only server: only `2049/TCP` is exposed and mounts must use a v4 minor version. | `true`                          |
